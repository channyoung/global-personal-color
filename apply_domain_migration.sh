...
# ==============================================================
# Find Your Personal Color -- DOMAIN MIGRATION
# Replaces global-personal-color.vercel.app with
# findyourpersonalcolor.com across ALL files (canonical, hreflang,
# OG, JSON-LD, sitemap.xml -- 679 occurrences across 102 files),
# plus adds robots.txt (was missing from prior batches).
# This is a full site re-sync: all 114 files included to
# guarantee zero stale references. Run from repo root.
# ==============================================================

echo "Re-syncing entire site for domain migration..."
mkdir -p en
mkdir -p en/guide
mkdir -p es
mkdir -p es/guide
...
cat > robots.txt <<'EOF_GPC'
User-agent: *
Allow: /

Sitemap: https://findyourpersonalcolor.com/sitemap.xml
EOF_GPC
...
git add -A
git commit -m "Domain migration: global-personal-color.vercel.app -> findyourpersonalcolor.com across all pages, sitemap, robots.txt"
git push

echo "Done. Verify with: curl -s https://findyourpersonalcolor.com/sitemap.xml | grep -c findyourpersonalcolor.com"
